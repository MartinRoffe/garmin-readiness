...
| `text_cache` | SQLite table | Workout descriptions, metric explainers, recovery suggestions, fuelling plans, weekly briefings (key: `weekly_briefing_v2_{monday_iso}`), HR stage plans (key: `hr_stage_plan_v1_{day}`) | `DELETE FROM text_cache WHERE key = '...'` (stage plans: `WHERE key LIKE 'hr_stage_plan_v1_%'`) |
