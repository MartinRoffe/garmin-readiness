...
- `modulation.py` — HRV-guided traffic light. `hrv_traffic_light(m, comp_z)` classifies the day green/amber/red/unknown from last-night HRV z-score vs 30-day baseline (+ 7d/30d ratio + composite backstop). `session_modulation(target, m, comp_z, light=)` turns amber/red into a concrete session swap (amber: `EASIER_VARIANT` map, duration kept; red: Recovery Spin 30 min) applied via the existing `/apply-plan-change` override flow. Returns None when green/unknown, rest day, or an override already exists. **Currently covers only `session_for_date_extended` (12-week plan + Tenerife + event prep + charity ride), NOT `hr_session_for_date`** — see TODO below.

  **TODO — extend HRV modulation to the Haute Route plan (before `HR_PLAN_START` = 2026-10-05).** `session_modulation` calls `session_for_date_extended(target)`, which returns None for HR-plan dates, so from October it only renders a "pill" with no swap. To cover the HR plan: (1) fall back to `hr_session_for_date(target)` in `session_modulation` when `session_for_date_extended` returns None; (2) add `EASIER_VARIANT` entries for the HR-only session types — design decision required for what each amber swap *becomes*, suggested: `vo2`→`("endurance","Z2 Endurance")`, `sweetspot`→`("endurance","Z2 Endurance")`, `endurance`→`("endurance","Z2 Easy")`, `back_to_back`→`("long","Long Ride (Easy)")` (keep duration), `recovery`/`gym`→ no swap (already easy); red day on any → Recovery Spin 30 min as today; (3) make `hr_session_for_date()` **and** `build_hr_calendar_weeks()` honour `plan_overrides` (they currently iterate `HR_TRAINING_WEEKS` directly and ignore overrides), otherwise an accepted swap is saved to `plan_overrides` but the HR calendar never reflects it. Safe to do: `_hr_ctl_projection` iterates `HR_TRAINING_WEEKS` directly, so making `hr_session_for_date` override-aware does **not** change projected CTL.
